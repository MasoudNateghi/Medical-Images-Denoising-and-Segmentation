...
if(Options.Verbose)
    h=figure; set(h,'render','opengl')
     subplot(2,2,1),
      ...
      title('The image with initial contour', "Interpreter","latex")
     subplot(2,2,2),
      imshow(Eext,[]); 
      title('The external energy', "Interpreter","latex");
     subplot(2,2,3), 
      ...
      title('The external force field ', "Interpreter","latex")
     subplot(2,2,4), 
      imshow(I), hold on; plot(P(:,2),P(:,1),'b.'); 
      title('Snake movement ', "Interpreter","latex")
    drawnow
end
